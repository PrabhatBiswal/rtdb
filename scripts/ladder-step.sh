...
export # The REGISTRY's names, which are the user's console typos and not the sidebar read of them:
# `hundsonhonet` and `matter`. Step 1 died at hello with 4401 on clients 0 and 3 for exactly this,
# twice, and the pre-flight leader list below is what shows the truth — read it before you trust
# this default (2026-09-11, WORKLOAD §5.30).
LOADSIM_NS_LIST=${LOADSIM_NS_LIST:-hundsonhonet,lightingmacqueen,lily,matter}
OUT=${OUT:-$(mktemp -d)}
...
prom() {
  ...
  local cid j
  j=$(mktemp)
  Q="$q" OUT_J="$j" INST="$OPS" python3 -c '
...
echo "  1 pool_waiting:";       prom "max_over_time(rtdb_pg_pool_waiting[${WINDOW}s])"
echo "  2 connections_pending:";prom "max_over_time(rtdb_connections_pending[${WINDOW}s])"
echo "  3 ack p99 (s):";        prom "histogram_quantile(0.99, sum by (le) (rate(rtdb_ack_seconds_bucket[${WINDOW}s])))"
echo "  4 quota_rejected /db:"; prom "increase(rtdb_quota_rejected_total[${WINDOW}s]) > 0"
...
echo "  5 RSS % of 4 GiB:";     prom "max by (instance) (rtdb_proc_process_resident_memory_bytes) / (4*1024*1024*1024) * 100"
...
echo "  6 acq/s per db:";       prom "sum by (db) (rate(rtdb_lock_acquisitions_total[${WINDOW}s]))"
# C1's wall was delivery, not writes, and this is the delivery counter: frames leaving a sink.
echo "  7 deliveries/s per gw:";prom "sum by (instance) (rate(rtdb_fanout_seconds_count[${WINDOW}s]))"
echo "  8 CPU cores per gw:";   prom "sum by (instance) (rate(rtdb_proc_process_cpu_seconds_total[${WINDOW}s]))"
echo "  9 consumer lag max:";   prom "max_over_time(rtdb_consumer_lag_revs[${WINDOW}s])"
echo " 10 event-loop lag max:"; prom "max_over_time(rtdb_proc_nodejs_eventloop_lag_seconds[${WINDOW}s])"
echo " 11 up, min over window:";prom "min_over_time(sum(up{job=\"rtdb-gateway\"})[${WINDOW}s:15s])"
echo "  connections per db:";   prom "sum by (db) (rtdb_connections)"
echo "  storage bytes per db:"; prom "max by (db) (rtdb_storage_bytes)"
